...
class smallz4_original
{
  public:
   ...
   static void lz4(GET_BYTES getBytes, SEND_BYTES sendBytes, unsigned short maxChainLength = MaxChainLength,
   ...
   /// compress everything in input stream (accessed via getByte) and write to output stream (via send)
   static void lz4(GET_BYTES getBytes, SEND_BYTES sendBytes, unsigned short maxChainLength,
                   ...
                   void* userPtr = NULL)
   {
      smallz4_original obj(maxChainLength);
      obj.compress(getBytes, sendBytes, dictionary, useLegacyFormat, userPtr);
   }

   /// version string
   static const char* const getVersion() { return "1.5"; }
  ...
  private:
   ...
   explicit smallz4_original(unsigned short newMaxChainLength = MaxChainLength)
   ...
   void compress(GET_BYTES getBytes, SEND_BYTES sendBytes, const std::vector<unsigned char>& dictionary,
                 bool useLegacyFormat, void* userPtr) const
   {
      ...
      while (true) {
         // ==================== start new block ====================
         // first byte of the currently processed block (std::vector data may contain the last 64k of the previous
         // block, too)
         const unsigned char* dataBlock = NULL;

         // prepend dictionary
         if (parseDictionary) {
            // resize dictionary to 64k (minus 1 because we can only match the last 65535 bytes of the dictionary =>
            // MaxDistance)
         ...
         if (useLegacyFormat) {
            ...
            // clear hash tables
            for (size_t i = 0; i < previousHash.size(); i++) previousHash[i] = EndOfChain;
            for (size_t i = 0; i < previousExact.size(); i++) previousExact[i] = EndOfChain;
